...
def publish_skin(request):
    ...
    Expected POST (multipart/form-data or JSON):
      - skin_name
      - creator
      - skin_code
      - svg (raw svg string OR uploaded file)
    """
    ...
    # Validation
    # --------------------
    if not skin_name or not creator or not skin_code:
        return JsonResponse(
            {"error": "skin_name, creator, and skin_code are required"},
            status=400,
        )
    ...
    if len(skin_name) > 1000:
        return JsonResponse(
            {"error": "Skin name must be under 1000 characters"},
            status=400,
        )
    ...
    try:
        ...
        return JsonResponse(
            {"error": f"Failed to read SVG content: {e}"},
            status=400,
        )
    ...
    skin = Skin.objects.create(
        ...
        creator=creator,
        skin_code=skin_code,
        image_url="",  # set after files are saved
        uuid=uuid.uuid4(),
    )
    ...
    try:
        ...
        # Update image_url to SVG (canonical)
        ...
        return JsonResponse(
            {"error": f"Failed to generate skin images: {e}"},
            status=500,
        )

    # --------------------
    # Save SkinImage records
    # --------------------
   ...
   # --------------------
    ...
    share_url = request.build_absolute_uri(
        reverse(
            "share_skin",
            kwargs={"skin_id": skin.id, "uuid": skin.uuid},
        )
    )
