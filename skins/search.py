...
def search_skins(request):
    ...
    if query:
        skins_queryset = Skin.objects.annotate(
            search=SearchVector("name", "creator", "description")
        ).filter(search=query).order_by("creator", "name")
    else:
        skins_queryset = Skin.objects.all()
        skins_list = list(skins_queryset)
        random.shuffle(skins_list)
        skins_list = skins_list[:50]

        # Skip pagination for homepage random results
        skins_data = [
            {
                "name": skin.name,
                "creator": skin.creator,
        ...
        return render(request, "skins/search.html", {
            ...
            "query": query,
            'daily_skin_count': Skin.objects.filter(
                created_at__range=_get_user_today_range(request)
            ).count(),
            'total_skin_count': Skin.objects.count(),
        })

    # Only paginate *after* sorting in the DB
    paginator = Paginator(skins_queryset, per_page)
    page_obj = paginator.get_page(page_number)
...
def _get_user_today_range(request):
    """Helper function to compute UTC start and end of user's local today."""
    now_utc = timezone.now()
    try:
        tz_offset_minutes = int(request.GET.get("tz_offset", "0"))
    except ValueError:
    ...
    return (
        user_today_start + timedelta(minutes=tz_offset_minutes),
        user_today_end + timedelta(minutes=tz_offset_minutes)
    )
